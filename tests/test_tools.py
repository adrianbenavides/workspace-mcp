    ...
    return {
        "execution": {
            ...
            "allowed_commands": [
                ...
                {"name": "ls", "binary": "/bin/ls", "allowed_arguments_regex": ["^-[la]{1,2}$"]},
                {"name": "sleep", "binary": "/bin/sleep", "allowed_arguments_regex": ["^[0-9]+$"]},
                {"name": "no-args", "binary": "/bin/echo"},
            ],
        }
    }
...
async def test_execute_job_env_cleaning() -> None:
    config_with_env = {
        "execution": {
            "default_timeout_seconds": 2.0,
            "allowed_commands": [
                {"name": "env", "binary": "/usr/bin/env", "allowed_arguments_regex": ["^$"]}
            ],
        }
    }
